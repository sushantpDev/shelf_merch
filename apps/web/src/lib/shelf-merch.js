...
================================================================= */
  ...
  function backLink(label, act, arg, opts = {}) {
    ...
    const argAttr =
      arg !== undefined && arg !== null && arg !== "" ? ` data-arg="${esc(arg)}"` : "";
...
const defaultState = {
  ...
  org:{
    ...
    collections: [
      {
        ...
        products: [
          { g: "pack", brand: "Mercer+Mettle", nm: "Mercer+Mettle Pack" },
          { g: "hoodie", brand: "Bella + Canvas", nm: "Bella + Canvas Youth Sponge Fleece" },
          { g: "mug", brand: "", nm: "Black Glossy Mug 11oz" },
          { g: "bottle", brand: "", nm: "The Standard Bottle" },
        ],
      },
    ],
    kits: [
    ...
    contacts: [
      ...
      },
      {
        ...
        name: "Chandra Sekhar",
        role: "Sender",
        address: "Flat No 1004, Tower-9, Eipl Corner Stone",
        loc: "Hyderabad, Telangana, IN",
      },
    ],
    departments: [
      {
        ...
        managerEmail: "konetibaba@gmail.com",
      },
      {
        id: "d2",
        walletId: "w1",
        ...
        budget: 90000,
        spent: 38000,
        manager: "Jonna Madhavi",
        managerEmail: "jonnaml2015@gmail.com",
      },
      {
        id: "d3",
        walletId: "w1",
  ...
  const S =
    typeof window !== "undefined" && window.__shelfMerchState
      ? window.__shelfMerchState
      : {
          ...defaultState,
          org: {
            ...defaultState.org,
            departments: defaultState.org.departments.map((d) => ({ ...d, mgr: { ...d.mgr } })),
          },
    ...
    if (n === "catalog" && !api.useMocks() && api.isAuthenticated()) {
      ...
      try {
        ...
        const catalog = await api.refreshCatalogProducts(
          selected === "All Products" ? undefined : selected,
        );
...
function bindSwNameInput(){
  ...
  function getLogoUploadsList() {
    ...
    for (const s of S.shops || []) {
      if (s.logoUrl)
        add({ name: (s.name || "Shop") + " logo", logoUrl: s.logoUrl, ext: "LOGO", size: 0 });
    }
    return out;
  }
  function ViewFor(v) {
    const map = {
      ...
      billing: () => Stub("Billing", "Manage invoices, GST details and payment methods.", I.bill),
  ...
  function ViewSettings() {
    ...
    ${[
      ...
      .map(
        ([k, t]) =>
          `<div class="item ${tab === k ? "on" : ""}" data-act="setTab" data-arg="${k}">${t}</div>`,
      )
      .join("")}</div>`;
  ...
  function saveWorkspace() {
    const n = document.getElementById("ws-name");
    if (n) S.account = n.value || S.account;
  ...
  function openModal(html) {
    document.getElementById("layer").innerHTML =
      `<div class="scrim" data-scrim><div class="modal scroll">${html}</div></div>`;
  }
  function openDrawer(html) {
    document.getElementById("layer").innerHTML =
      `<div class="drawer-scrim" data-scrim></div><div class="drawer">${html}</div>`;
  }
  function closeLayer() {
    document.getElementById("layer").innerHTML = "";
  }
  ...
  function toast(msg, ok = true) {
    ...
    e.innerHTML =
      (ok ? I.check.replace("<svg ", '<svg width="18" height="18" ') : "") +
      `<span>${esc(msg)}</span>`;
    host.appendChild(e);
  ...
  Wizards.createKit = function () {
    ...
    if (step === 0) {
      ...
      <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(180px,1fr))">${getCatalogList()
        .map((p, i) => {
    ...
    const back =
      step > 0
        ? backLink("Back", "ktBack", null, { mb: "0" })
        : backLink("Cancel", "wzExit", null, { mb: "0" });
    const next =
      step < 3
        ? `<button type="button" class="btn btn-dark" ${step === 1 && !f.picked.length ? "disabled" : ""} data-act="ktNext">Next</button>`
  ...
  function kitLogoImg(f) {
    ...
    if (url)
      return `<img src="${url}" alt="Kit logo" style="max-width:100%;max-height:100%;object-fit:contain;display:block">`;
    return "";
  }
  function kitBrandingPanel(f) {
    ...
    <div><div class="${hasLogo ? "banner info" : "banner"}" style="margin-bottom:16px">${hasLogo ? "<div>Branding applied. Our team will share proofs within 2 business days.</div>" : "<div>Add a logo to preview branded mockups for each item.</div>"}</div>
      <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(170px,1fr))">${prods
        .map((p, idx) =>
          hasLogo
            ...
            : pcard(p, { branded: hasLogo, artworkUrl: f.logoFile?.preview, act: "noop" }),
        )
        .join("")}</div></div></div>`;
  }
  ...
  function ktLogoSetFile(file) {
    if (!LOGO_ACCEPT.test(file.name)) {
      toast("Accepted formats: SVG, PNG, WEBP, JPEG, JPG", false);
      return;
    }
    if (file.size > LOGO_MAX) {
      toast("File must be 5 MB or smaller", false);
      return;
    }
  ...
  function ktLogoUpload() {
    document.getElementById("kt-logo-inp")?.click();
  }
  function ktLogoClear() {
    S.flow.logoFile = null;
    render();
  };
  reader.readAsDataURL(file);
}
...
async function kitPublish(){
  ...
  function ktPkg(el) {
    S.flow.pkg = el.dataset.arg;
    render();
  }
  function ktBack() {
    S.flow.step = Math.max(ktFlowStep(S.flow) - 1, 0);
    render();
  }
  let _ktNextLock = 0;
  function ktNext() {
    ...
    const step = ktFlowStep(f);
    if (step === 0) f.kitName = document.getElementById("kt-name")?.value || "New Kit";
    if (step === 1 && !f.picked?.length) {
      toast("Select at least one product", false);
      return;
    }
    if (step === 2) f.notes = (document.getElementById("kt-notes") || {}).value || "";
    f.step = Math.min(step + 1, 3);
  ...
  async function kitPublish() {
    ...
    if (api.useMocks()) {
      const catalog = getCatalogList();
      const productRefs = f.picked
        .map((i) => {
          const p = catalog[i];
          return p
            ? { catalogProductId: p.id || "", name: p.nm, brand: p.brand || "", group: p.g }
            : null;
        })
        .filter(Boolean);
      const k = {
        id: nid("k"),
        name: f.kitName,
        items: f.picked.length,
        status: "live",
        sent: false,
        artworkUrl: f.logoFile?.preview || "",
        designNotes: f.notes || "",
        picked: f.picked.slice(),
        productRefs,
      };
      S.kits.push(k);
      toast('Kit "' + k.name + '" published');
  ...
  function kitPickedIndices(kit) {
    ...
    if (Array.isArray(kit.productRefs) && kit.productRefs.length) {
      ...
      for (const ref of kit.productRefs) {
        let idx = catalog.findIndex(
          (p) => p.id && ref.catalogProductId && p.id === ref.catalogProductId,
        );
        if (idx < 0 && ref.name)
          idx = catalog.findIndex(
            (p) => p.nm === ref.name && (p.brand || "") === (ref.brand || ""),
          );
  ...
  function editKitStart(id) {
    ...
    S.flow = {
      ...
      kitId: id,
      kitName: k.name,
      picked: kitPickedIndices(k),
      step: 0,
      logoFile: kitLogoFromKit(k),
      notes: k.designNotes || "",
    };
    go("editKit");
  }
  ...
  Wizards.editKit = function () {
    ...
    if (step === 0) {
      ...
      const selectedCards = selected.length
        ...
        <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(160px,1fr))">${selected
          .map(
            (
              p,
              si,
            ) => `<div class="pcard" style="position:relative"><div class="img">${productImg(p)}</div><div class="meta">${p.brand ? `<div class="brand">${esc(p.brand)}</div>` : ""}<div class="nm">${esc(p.nm)}</div></div>
          <button type="button" class="xbtn" style="position:absolute;top:8px;right:8px;background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.12)" data-act="ekRemove" data-arg="${f.picked[si]}" title="Remove">✕</button></div>`,
          )
          .join("")}</div></div>`
...
async function kitSaveEdit(){
  ...
  function sendItemsStart(el) {
    const id = el && el.dataset ? el.dataset.arg : null;
    closeLayer();
    const k = S.kits.find((x) => x.id === id);
    const picked = kitPickedIndices(k);
    if (!picked.length) {
      toast("This kit has no products — edit the kit to add items", false);
      return;
    }
  ...
  function sendItemsStartFor(kitId, picked, artworkUrl) {
    ...
    S.flow = {
      ...
      kitId,
      picked,
      artworkUrl: artworkUrl || k?.artworkUrl || "",
      selRecips: S.contacts.slice(0, 2).map((c) => c.id),
      mode: "redeem",
      singleLocation: {
        name: "",
        email: "",
  ...
  Wizards.sendItems = function () {
    ...
    if (step === 0) {
      ...
      body = recipientPicker(
        f,
        "Who's receiving this?",
        "Choose how recipients get their items, then pick people.",
        { modes: true },
      );
      if (f.mode === "surprise") {
        const missing = S.contacts.filter(
          (c) => f.selRecips.includes(c.id) && (!c.address || !c.city || !c.state || !c.pincode),
        );
        if (missing.length)
          body += `<div class="banner" style="margin-top:14px">${I.truck.replace('width="24" height="24"', 'width="16" height="16"')}<div><b>${missing.length} recipient(s) missing a shipping address.</b> Surprise sends need addresses up front. <span class="lnk" data-act="contactEdit" data-arg="${missing[0].id}">Fix recipient information</span></div></div>`;
      }
    } else if (step === 2) {
      body =
        recipientExperience(f, { kind: "items" }) +
        `
      ...
      const unit = 4200,
        qty = f.selRecips.length,
        sub = unit * qty;
      const pkgCost = (f.pkg === "box" ? 49 : 0) * qty;
      const fee = sub * 0.12;
      const ship = 120 * qty;
      const tax = (sub + fee + pkgCost + ship) * 0.18;
    ...
    const back =
      step > 0
        ? backLink("Back", "siBack", null, { mb: "0" })
        : backLink("Save draft", "wzExit", null, { mb: "0" });
    const next =
      step < 3
        ? `<button type="button" class="btn btn-dark" data-act="siNext">Next</button>`
        : "<span></span>";
    return wzChrome("Send Items", SI_STEPS, step, body, back + next);
  };
  function siBack() {
    S.flow.step = Math.max(siFlowStep(S.flow) - 1, 0);
    render();
  }
  let _siNextLock = 0;
  function siNext() {
    ...
    if (step === 1 && f.mode === "surprise") {
      ...
      if (missing.length) {
        toast("Complete the shipping address for all surprise recipients", false);
        return;
      }
    }
    if (step === 1 && f.mode === "single") {
      ...
      if (
        ...
        toast("Enter the location contact, email, and complete shipping address", false);
        return;
      }
    }
    if (step === 2) {
      const n = document.getElementById("si-note");
      if (n) f.note = n.value;
      const fr = document.getElementById("re-from");
      if (fr) f.from = fr.value;
      const m = document.getElementById("re-msg");
      if (m) f.msg = m.value;
    }
    f.step = Math.min(step + 1, 3);
    render();
  ...
  async function sendItemsDo() {
    ...
    if (api.useMocks()) {
      S.orders.unshift({
        ...
        name: k ? k.name : "Custom kit",
        status: "Processing",
        amount: 4200 * f.selRecips.length,
        track: "",
        items: f.picked.map((i) => [DEMO_PRODUCTS[i].nm, String(f.selRecips.length)]),
      });
      finishUi();
      render();
      return;
    ...
    const entityId = S.primaryEntityId || (S.org.departments[0] && S.org.departments[0].id);
    if (!entityId) {
      toast("No department budget found — complete wallet setup first");
      return;
    }
    if (!f.kitId) {
      toast("Kit not found — reload and try again");
      return;
    }
    if (!f.selRecips.length) {
    ...
    try {
      ...
      const campaign = await api.launchKitCampaignFlow({
        ...
        name: k ? k.name : "Kit send",
        fulfillmentMode:
          f.mode === "surprise" ? "surprise" : f.mode === "single" ? "single" : "redeem",
        singleLocation: f.mode === "single" ? f.singleLocation : undefined,
        message: { from: f.from || "", body: f.msg || "" },
        schedule: siScheduleFromFlow(f),
        contactIds: f.selRecips,
  ...
  function swagDesignerStart(el) {
    closeLayer();
    const shopId =
      (el && el.dataset && el.dataset.arg) || S.flow.shopId || (S.shops[0] && S.shops[0].id);
    S.flow = {
      exitTo: shopId ? "shopDetail" : "swag",
      shopId,
      colName: "New Employee Swag",
      picked: [],
      artwork: false,
      exitToNav: shopId ? "shops" : "swag",
    };
    go("swagName");
  }
      ...
      !filteredEntries.length
        ...
        ${filteredEntries
          .map(({ p, i }) => {
            ...
            const sw = productColorNames(ep)
              .slice(0, 6)
              .map(
                (c) =>
                  `<span class="sw" style="background:${productColorHex(ep, c)}" title="${esc(c)}"></span>`,
              )
              .join("");
  ...
  Wizards.swagArtwork = function () {
    ...
    if (atab === "device") {
      if (f.artwork && f.artFile) {
        ...
        const artMeta = esc(
          af.ext && af.size ? af.ext + " · " + fmtFileSize(af.size) : af.ext || "",
        );
    ...
    const summary = f.artwork
      ? `<div class="card" style="padding:18px;background:var(--surface-2);border-radius:var(--r-sm)">
    ...
    <div class="sw-mockups">${prods
      .map((p, idx) => {
        const ep = enrichProduct(p);
        const mock = productHasPrintArea(ep);
        const inner = f.artwork
          ? swagMockupHost(ep, idx)
          : `<div class="img${mock ? " img-mockup" : ""}">${productImg(ep, { ...(mock ? { width: "100%", height: "100%" } : {}), url: designImgUrl(ep) })}</div>`;
        const badge = f.artwork
          ? `<div class="mockup-badge">${I.spark.replace("<svg ", '<svg width="11" height="11" ')}Editable</div>`
          : "";
        return `<div class="pcard mockup-card" style="position:relative">${f.artwork ? "" : '<div class="dots-btn">' + I.dots + "</div>"}${badge}${inner}<div class="meta">${p.brand ? `<div class="brand">${esc(p.brand)}</div>` : ""}<div class="nm">${esc(p.nm)}</div></div></div>`;
      })
      .join("")}</div></div>`;
  ...
  function swArtImg(f, opts = {}) {
    ...
    if (url)
      return `<img src="${url}" alt="Artwork" style="max-width:100%;${opts.maxH ? `max-height:${opts.maxH};` : ""}object-fit:contain;display:block">`;
    return opts.fallback === false ? "" : LOGO_DECO;
  }
  function swArtSetFile(file) {
    if (!ART_ACCEPT.test(file.name)) {
      toast("Accepted formats: SVG, PNG, JPG, AI", false);
      return;
    }
  ...
  function swArtUpload() {
    document.getElementById("sw-art-inp")?.click();
  }
  function swArtClear() {
    S.flow.artwork = false;
    S.flow.artFile = null;
    S.flow.artSel = null;
    S.flow.artPlacements = {};
    render();
  }
  function swResetArt() {
    S.flow.artPlacements = {};
  ...
  function getFabricTexture() {
    ...
    x.globalAlpha = 0.05;
    x.strokeStyle = "#000"; // faint horizontal weave
  ...
  function buildRealisticArtwork(artImg, group) {
    try {
      ...
      const nw = artImg.naturalWidth || artImg.width || 0,
        nh = artImg.naturalHeight || artImg.height || 0;
  ...
  function mockupContext() {
    ...
    if (S.view === "swagArtwork") {
      ...
      return {
        products: f.pickedProducts || [],
        artUrl: f.artFile.preview,
        store: (f.artPlacements = f.artPlacements || {}),
      };
    }
    if (S.view === "createKit" || S.view === "editKit") {
      ...
      return {
        products,
        artUrl: f.logoFile.preview,
        store: (f.kitPlacements = f.kitPlacements || {}),
      };
    }
    return null;
  }
  ...
  function mockupPlacementKey(prod, idx) {
  ...
  async function mountOneMockup(host, prod, idx, ctx) {
    ...
    if (!size) {
      ...
      if (tries < 3) {
        host.dataset.kretry = String(tries + 1);
        requestAnimationFrame(() => mountKonvaMockups());
      }
      return;
    }
    ...
    const box =
      area && area.box && area.box.widthPct
        ? area.box
        : { xPct: 33, yPct: 30, widthPct: 34, heightPct: 38 };
    const bx = (box.xPct / 100) * size,
      by = (box.yPct / 100) * size;
    const bw = (box.widthPct / 100) * size,
      bh = (box.heightPct / 100) * size;
    layer.add(
      new Konva.Rect({
        x: bx,
        y: by,
...
async function swGenerate(){
  ...
  function sendPointsStart(el) {
    ...
    S.flow = {
      exitTo: shopId ? "shopDetail" : "shops",
      exitToNav: "shops",
      returnState,
      shopId,
      step: 0,
  ...
  Wizards.sendPoints = function () {
    ...
    if (step === 0) {
      ...
      body = recipientPicker(
        f,
        "Add recipients",
        "Don't have all emails? You can add recipients later from the shop dashboard.",
      );
    } else if (step === 2) {
      body = recipientExperience(f, { kind: "points" });
    } else {
      const sub = (f.ppr / PT) * f.recips * PT;
      const fee = sub * 0.15;
      const tax = (sub + fee) * 0.18;
...
async function sendPointsDo(){
  ...
  function sumRow(k, v) {
    return `<div class="row" style="justify-content:space-between;padding:7px 0"><span class="muted" style="font-size:13px">${k}</span><span class="num" style="font-weight:600;font-size:13px">${v}</span></div>`;
  }
  function paymentPanel() {
    const w = S.wallets[0];
    const bal = w ? w.balance : 0;
    const sel = S.flow.pay || "wallet";
    const opt = (k, title, sub, extra) =>
      `<div class="pay-opt ${sel === k ? "on" : ""}" data-act="payPick" data-arg="${k}"><div class="rd"></div><div style="flex:1"><div style="font-weight:600;font-size:13.5px">${title}</div><div class="mut3" style="font-size:11.5px">${sub}</div></div>${extra || ""}</div>`;
  ...
  function recipientPicker(f, title, sub, opts = {}) {
    const rows = S.contacts
      .map((c) => {
        const on = f.selRecips.includes(c.id);
        return `<tr><td><div data-act="recToggle" data-arg="${c.id}" style="width:18px;height:18px;border:2px solid ${on ? "var(--brand)" : "#c4ccc6"};border-radius:4px;display:grid;place-items:center;background:${on ? "var(--brand)" : "#fff"};cursor:pointer">${on ? '<span style="color:#fff;font-size:11px">✓</span>' : ""}</div></td>
    <td class="muted">${esc(c.email)}</td><td style="font-weight:600">${esc(c.name)}</td><td class="muted">${c.loc ? esc(c.loc) : "—"}</td></tr>`;
      })
      .join("");
    const modes = opts.modes
      ...
      ${[
        ...
        .map(
          ([k, t, d]) =>
            `<div class="optcard ${(f.mode || "redeem") === k ? "on" : ""}" data-act="recMode" data-arg="${k}"><div class="rd"></div><div><h4>${t}</h4><p>${d}</p></div></div>`,
        )
        .join("")}</div>`
      : "";
  ...
  function recToggle(el) {
    const id = el.dataset.arg;
    const a = S.flow.selRecips;
    const i = a.indexOf(id);
    if (i < 0) a.push(id);
    else a.splice(i, 1);
    render();
  }
  function recDeselect() {
    S.flow.selRecips = [];
    render();
  }
  function recMode(el) {
    S.flow.mode = el.dataset.arg;
    render();
  }
  function singleLocLive(el) {
    S.flow.singleLocation = S.flow.singleLocation || {};
    S.flow.singleLocation[el.dataset.k] = el.value;
  }
  ...
  function recipientExperience(f, opts = {}) {
    ...
    const eta = new Date(Date.now() + 5 * 86400000).toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
    });
    const items = f.picked ? f.picked.length : 3;
    const headline = isPoints
      ? `You've been gifted ${(f.ppr / PT).toFixed(0)} Pts to ${esc(shop)}!`
      : shipMode
  ...
  function reMsg(e) {
    S.flow.msg = e.target ? e.target.value : "";
    const l = document.getElementById("re-prev-l"),
      em = document.getElementById("re-prev-e");
    if (l) l.textContent = S.flow.msg;
    if (em) em.textContent = S.flow.msg;
  }
  function reNote(e) {
    S.flow.note = e.target ? e.target.value : "";
    const p = document.getElementById("si-prev");
    if (p) p.textContent = S.flow.note;
  }

  /* catalog list — live API data only (no demo fallback when connected to API) */
  function getCatalogList() {
    return api.useMocks() ? DEMO_PRODUCTS : S.catalogProducts || [];
  }
  ...
  function productCategory(p) {
    return PRODUCT_CATEGORIES[p.g] || "Merch";
  }
  ...
  function productUniqueId(col, pIdx) {
    const base = (col.code || "").replace(/\D/g, "").slice(-6) || "100000";
    return base + String(pIdx + 1).padStart(2, "0");
  }
  ...
  const DEMO_PRODUCTS = [
    {
      g: "tee",
      brand: "Port & Company",
      nm: "Youth Core Cotton Tee",
      price: "as low as ₹180",
      sw: 35,
      colors: ["Black", "White", "Navy", "Blue", "Red", "Green", "Gray"],
    },
    {
      g: "hoodie",
      brand: "Bella + Canvas",
      nm: "Sponge Fleece Pullover Hoodie",
      price: "as low as ₹1,150",
      sw: 6,
      colors: ["Black", "Navy", "Blue", "Green", "Gray", "Red"],
    },
    {
      g: "bottle",
      brand: "",
      nm: "The Standard Bottle",
      price: "₹890",
      sw: 4,
      colors: ["Black", "Blue", "Green", "Gray", "Navy"],
    },
    {
      g: "mug",
      brand: "",
      nm: "Black Glossy Mug 11oz",
      price: "₹420",
      sw: 3,
      colors: ["Black", "White"],
    },
    ...
    },
    {
      g: "power",
      brand: "Ambrane",
      nm: "Xtreme-10 Power Bank",
      price: "as low as ₹1,426",
      sw: 2,
    ...
    },
    {
      g: "pillow",
      brand: "",
      nm: "Travel Neck Pillow",
      price: "₹540",
      sw: 6,
      colors: ["Black", "Gray", "Blue", "Navy"],
    },
    {
      g: "bag",
      brand: "ChangeBag",
      nm: "Organic Canvas Tote",
      price: "₹360",
      sw: 7,
    ...
    },
    {
      ...
      nm: "Garment-Dyed Heavyweight Tee",
      price: "₹1,640",
      sw: 61,
      colors: ["Black", "White", "Navy", "Blue", "Red", "Green", "Gray", "Yellow"],
    },
    {
      g: "bottle",
      brand: "DeskMate",
      nm: "Steel Bottle 750ml",
      price: "₹689",
      sw: 5,
      colors: ["Black", "Blue", "Green", "Gray", "Navy"],
    },
  ...
  Wizards.createShop = function () {
    const f = S.flow;
    const step = f.step | 0 || 0;
    if (step === 0) {
      const cur = f.shopCur || "Points";
      const opt = (k, t, d) =>
        `<button type="button" class="optcard ${cur === k ? "on" : ""}" data-act="shopCur" data-arg="${k}" style="margin-bottom:10px;width:100%;text-align:left"><div class="rd"></div><div><h4>${t}</h4><p>${d}</p></div></button>`;
    ...
    if (ltab === "prev") {
      ...
      const logoName = esc(lf.name || "logo");
      const logoMeta = esc(
        lf.sizeLabel || (lf.ext && lf.size ? lf.ext + " · " + fmtFileSize(lf.size) : lf.ext || ""),
      );
  ...
  function createShopStart() {
    S.flow = { exitTo: "shops", step: 0, shopCur: "Points", shopName: "" };
    go("createShop");
  }
  function shopCur(el) {
    readCreateShopName();
  ...
  function shopLogoUpload() {
    document.getElementById("sh-logo-inp")?.click();
  }
  function shopLogoClear() {
    S.flow.logo = false;
    S.flow.logoFile = null;
    render();
  }
  function shopBuildGo() {
    go("shopBuilder");
  }
  ...
  function shopBannerBase(t) {
    return t.bg.startsWith("linear-gradient") ? t.bg : `linear-gradient(${t.bg},${t.bg})`;
  }
  function shopBannerStyle(f) {
    const t = shopBannerCfg(f);
    if (t.dots)
      return `background-image:radial-gradient(rgba(255,255,255,.18) 1.4px,transparent 1.4px),${shopBannerBase(t)};background-size:14px 14px,auto;`;
    return `background:${t.bg};`;
  }
  function shopLogoImg(src) {
    const url = src?.logoFile?.preview || src?.logoUrl;
    if (url)
      return `<img src="${url}" alt="Shop logo" style="max-width:100%;max-height:100%;object-fit:contain;display:block">`;
    return LOGO_DECO;
  }
  function shopThemeKey(src) {
    return src?.bannerTheme || src?.bannerConfig?.theme || "light";
  }
  function shopBannerCfg(src) {
    return BANNER_THEMES[shopThemeKey(src)] || BANNER_THEMES.light;
  }
  function shopBannerClasses(src) {
    const t = shopBannerCfg(src);
    return "shopbanner" + (t.dots ? " shopbanner-merch" : "");
  }
  function shopBannerHtml(src, opts = {}) {
    const h = opts.height || 96,
      logo = opts.logoSize || 48,
      layout = opts.layout || "left",
      r = opts.radius ?? 0;
    const logoPos =
      layout === "center"
  ...
  function bannerThemePicker(cur, act) {
    return Object.keys(BANNER_THEMES)
      .map((k) => {
        const t = BANNER_THEMES[k];
        const sw = t.dots
          ? `background-image:radial-gradient(rgba(255,255,255,.18) 1.4px,transparent 1.4px),${shopBannerBase(t)};background-size:14px 14px,auto;`
          : `background:${t.bg};`;
      ...
      <div style="font-size:12px;margin-top:8px;font-weight:600;text-transform:capitalize">${k}</div></button>`;
      })
      .join("");
  }
  function shopEditPreview(ed) {
    const s = S.shops.find((x) => x.id === ed.shopId) || {};
    return shopBannerHtml(
      {
        ...s,
        bannerConfig: { theme: ed.bannerTheme || "light" },
        logoUrl: ed.logoFile?.preview || ed.logoUrl || "",
      },
      { height: 108, layout: "center", logoSize: 48, radius: 10 },
    );
  }
...
async function shopEditSave(){
  ...
  function shopEditLogoClear() {
    if (!S.flow.shopEdit) return;
    S.flow.shopEdit.logoFile = null;
    S.flow.shopEdit.logoUrl = "";
    renderShopEditModal();
  }
  async function shopEditSave() {
    const ed = S.flow.shopEdit;
...
async function shopPublish(){
  ...
  function printAreaWrapStyle(box) {
    const fit =
      "box-sizing:border-box;overflow:hidden;display:flex;align-items:center;justify-content:center;min-width:0;min-height:0;pointer-events:none";
  ...
  function designedMockupHtml(ep, artworkUrl, fill) {
    if (ep?.mockupUrl)
      return `<img src="${esc(ep.mockupUrl)}" alt="" style="width:100%;height:100%;object-fit:contain;display:block">`;
  ...
  function productImg(p, opts = {}) {
    ...
    const size = opts.width
      ? `width:${opts.width};height:${opts.height || opts.width};`
  ...
  function pcard(p, opts = {}) {
    const sw = opts.swatches
      ? `<div class="swatches">${["#1c1c1c", "#2b4a8b", "#9a9a9a", "#7a4a25"].map((c) => `<span class="sw" style="background:${c}"></span>`).join("")}<span class="mut3" style="font-size:11px;align-self:center;margin-left:2px">+${opts.swatches}</span></div>`
      : "";
    const prod = enrichProduct(opts.product || p);
    const mockup = opts.branded || productHasPrintArea(prod);
    const inner = opts.branded
      ? designedMockupHtml(prod, opts.artworkUrl, mockup)
      : productImg(prod, mockup ? { width: "100%", height: "100%" } : {});
...
function ViewShopDetail(){
  ...
  function shopTabBody(s, tab) {
    if (tab === "Branded Swag") {
      ...
      const rail = `<div class="subrail">
      ${[
        ["Saved Designs", activeCols.length],
        ["Locker Inventory", ""],
        ["Archived", archivedCols.length],
      ]
        .map(
          ([k, ct]) =>
            `<div class="item ${sub === k ? "on" : ""}" data-act="swSub" data-arg="${k}">${k}${ct !== "" ? `<span class="ct">${ct}</span>` : ""}</div>`,
        )
        .join("")}</div>`;
      let content;
...
function swagEmptyDesigner(){
  ...
  function swagProductCount(cols) {
    return swagProductEntries(cols).length;
  }
  ...
  function swagAddToShopPick(el, shopId) {
    ...
    if (modal) {
      modal
        .querySelectorAll(".shop-pick-card")
        .forEach((c) => c.classList.toggle("on", c.dataset.arg === shopId));
  ...
  function syncShopCollections() {
    for (const shop of S.shops) {
      shop.collections = S.collections
        .filter((c) => collectionLinkedToShop(c, shop.id))
        .map((c) => c.id);
    }
  }
  function catalogIndexForProduct(p, catalog) {
    ...
    return catalog.findIndex(
      (x) => x.g === p.g && x.nm === p.nm && (x.brand || "") === (p.brand || ""),
    );
  }
  async function swagAddToShopDo() {
    ...
    if (!col || !shop || (!collectionMode && !ref)) {
      closeLayer();
      return;
    }
    const p = ref?.p;
    const duplicate = collectionMode
      ? collectionLinkedToShop(col, shop.id) ||
        col.products.some((product) => shopHasDesign(shop.id, col, product))
    ...
    if (duplicate) {
      ...
      toast(
        collectionMode
          ? "This collection or one of its products is already in this shop"
          : "This product is already in this shop",
        false,
      );
      render();
      return;
    ...
    try {
      if (api.useMocks()) {
        ...
        toast(
          collectionMode
            ? "Collection added to shop successfully!"
      ...
      if (linkCollection) {
        ...
        const created = await api.addProductToShopFlow({
          shopId: shop.id,
          collection: col,
          product: p,
          catalog,
  ...
  function swagDesignCard(col, p, pIdx) {
    ...
    const more =
      names.length > 4
        ? `<span class="mut3" style="font-size:10px;align-self:center">+${names.length - 4}</span>`
        : "";
...
async function collectionArchive(colId){
  ...
  function ViewSwag() {
    ...
    const tabLabels = {
      "All Products": `All Products (${count})`,
  ...
  function ViewProductDetail() {
    ...
    const colors = isCatalog
      ? colorNames.map((name) => productColorHex(p, name))
      : collectionProductSwatches(col, p);
    const sel = Math.min(S.flow.selColor || 0, colors.length - 1);
    const desc = isCatalog ? String(p.description || "") : productDescription(p);
    const short =
      desc.length > 180 && !S.flow.descExpanded ? desc.slice(0, 180).trim() + "…" : desc;
    ...
    const detailTabs = isCatalog
      ? `<div class="pd-detail-tabs" role="tablist" aria-label="Product information">
    ${[
      ...
      .map(
        ([key, label]) =>
          `<button type="button" class="${detailTab === key ? "on" : ""}" role="tab" aria-selected="${detailTab === key ? "true" : "false"}" data-act="productDetailTab" data-arg="${key}">${label}</button>`,
      )
      .join("")}
  </div>`
      : "";
    ...
    const backLabel = isCatalog
      ? "Back to catalog"
      : S.flow.productBackView === "shopDetail"
        ? "Back to shop"
        : "Back to saved designs";
    const uniqueId = isCatalog
      ? p.id
        ? String(p.id).slice(-8).toUpperCase()
        : String(pIdx + 1).padStart(8, "0")
      : productUniqueId(col, pIdx);
    const notes = isCatalog
      ? "Ready to customize with your artwork"
      : `Internal notes for ${esc(col.name)}`;
  ...
  function productOpen(arg) {
    ...
    const pIdx = +pIdxStr;
    const col = S.collections.find((c) => c.id === colId);
    if (!col || !col.products[pIdx]) {
      toast("Product not found", false);
      return;
    }
    ...
    go("productDetail", {
      ...
      flow: {
        ...
        colId,
        pIdx,
        selColor: primaryColorSel(),
        descExpanded: false,
        productBackView: backView,
        productBackFlow: backFlow,
      },
    });
  }
  function catalogProductOpen(arg) {
    ...
    go("productDetail", {
      ...
      flow: {
        ...
        productBackView: "catalog",
        productBackFlow: { catCat: S.flow.catCat || "All Products" },
      },
    });
  }
  function productBack() {
    ...
    delete flow.colId;
    delete flow.pIdx;
    delete flow.selColor;
    delete flow.descExpanded;
    delete flow.productDetailTab;
    delete flow.productSource;
    delete flow.catalogProductId;
    delete flow.catalogProductIndex;
    delete flow.productBackView;
    delete flow.productBackFlow;
  ...
  function productDescToggle() {
    S.flow.descExpanded = !S.flow.descExpanded;
    render();
  }
  ...
  function productPurchase() {
    toast("Checkout coming soon — payment will be available in a future update");
  }
  ...
  function kitOpen(id) {
    ...
    const prodList = prods.length
      ? `<div style="margin:12px 0 16px"><div class="mut3" style="font-size:10px;letter-spacing:.08em;text-transform:uppercase;font-weight:700;margin-bottom:10px">Included products</div>
    <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:12px">${prods
      .map((p) => {
        const ep = enrichProduct(p);
        const mock = hasBrand || productHasPrintArea(ep);
        const inner = hasBrand
          ? designedMockupHtml(ep, k.artworkUrl, mock)
          : productImg(ep, mock ? { width: "100%", height: "100%" } : {});
        return `<div class="pcard" data-act="noop"><div class="img${mock ? " img-mockup" : ""}">${inner}</div></div>`;
      })
      .join("")}</div></div>`
      : "";
    const artBlock = k.artworkUrl
      ? `<div style="margin:12px 0 16px"><div class="mut3" style="font-size:10px;letter-spacing:.08em;text-transform:uppercase;font-weight:700;margin-bottom:8px">Kit artwork</div>
  ...
  /* ===================== CONTACTS ===================== */
  function roleSel(c) {
    return `<select class="inp" style="height:34px;width:auto;padding:0 30px 0 12px;display:inline-block" data-act="noop">${["Owner", "Admin", "Sender", "Member", "Non-Member"].map((r) => `<option ${c.role === r ? "selected" : ""}>${r}</option>`).join("")}</select>`;
  }
  function ViewContacts() {
    ...
    <td style="text-align:right"><button class="iconbtn" style="width:30px;height:30px" data-act="contactEdit" data-arg="${c.id}">${I.edit}</button></td></tr>`,
      )
      .join("");
    ...
    const stageLabel =
      stage === "uploading"
        ...
        : stage === "queued"
          ...
          : stage === "processing"
            ? "Validating and importing contacts…"
            : "Finishing up…";
  ...
  function renderAcImportPanel() {
    ...
    if (importFile) {
      ...
      if (parsing) {
        panel += acImportStatusRow(
          acImportSpinner(),
          "Reading file…",
          "Checking format and counting rows",
          true,
        );
        ...
        if (preview.rowCount != null) {
          const warn =
            preview.hasEmail === false
              ? "No email column found — import may fail"
              : preview.hasName === false
                ? "Name column missing — emails will be used as names"
                : "";
          panel += acImportStatusRow(
            acImportCheckIcon(),
            `${preview.rowCount} contact row${preview.rowCount === 1 ? "" : "s"} ready to import`,
            warn,
            false,
          );
          ...
          panel += acImportStatusRow(
            acImportCheckIcon(),
            "Excel file ready",
            "Rows will be validated when you import",
            false,
          );
        }
      ...
      if (result) {
        ...
        ${
          result.errors?.length
            ? `<div style="margin-top:8px;max-height:120px;overflow:auto;font-size:12px;color:var(--ink-2)">${result.errors
                .slice(0, 5)
                .map((e) => `Row ${e.row}: ${esc(e.message)}`)
                .join(
                  "<br>",
                )}${result.errors.length > 5 ? `<br>…and ${result.errors.length - 5} more` : ""}</div>`
            : ""
        }
      </div>`;
      }
  ...
  function renderAddContacts() {
    ...
    const submitLabel =
      tab === "csv"
        ? S.flow.importBusy
          ? "Importing…"
          : S.flow.importParsing
            ? "Reading file…"
            : "Import contacts"
        : "Add contacts";
    const submitDisabled =
      tab === "csv" && (S.flow.importBusy || S.flow.importParsing || !importReady);
    ...
    <div class="tabs" style="max-width:280px;margin:12px 0 18px">${[
      ["manual", "Manually"],
      ["csv", "Upload file"],
    ]
      .map(
        ([k, l]) =>
          `<button class="${tab === k ? "on" : ""}" data-act="acTab" data-arg="${k}">${l}</button>`,
      )
      .join("")}</div>
    ${body}
  ...
  async function addContactsDo() {
    ...
    const firstName = document.getElementById("ac-first")
      ? document.getElementById("ac-first").value.trim()
      : "";
    const lastName = document.getElementById("ac-last")
      ? document.getElementById("ac-last").value.trim()
      : "";
    const email = document.getElementById("ac-email")
      ? document.getElementById("ac-email").value.trim()
      : "";
    const phone = document.getElementById("ac-phone")
      ? document.getElementById("ac-phone").value.trim()
      : "";
    const role = document.getElementById("ac-role")
      ? document.getElementById("ac-role").value
      : "Member";
    const department = document.getElementById("ac-dept")
      ? document.getElementById("ac-dept").value.trim()
      : "";
    const employeeCode = document.getElementById("ac-emp-code")
      ? document.getElementById("ac-emp-code").value.trim()
      : "";
    const line1 = document.getElementById("ac-address")
      ? document.getElementById("ac-address").value.trim()
      : "";
    const city = document.getElementById("ac-city")
      ? document.getElementById("ac-city").value.trim()
      : "";
    const state = document.getElementById("ac-state")
      ? document.getElementById("ac-state").value.trim()
      : "";
    const pincode = document.getElementById("ac-pin")
      ? document.getElementById("ac-pin").value.trim()
      : "";
    const country = document.getElementById("ac-country")
      ? document.getElementById("ac-country").value
      : "IN";
    ...
    if (api.useMocks()) {
      ...
      closeLayer();
      toast("Added contact successfully");
      render();
      return;
    }
    try {
  ...
  function acImportPick() {
    document.getElementById("ac-import-inp")?.click();
  }
  ...
  async function acImportSetFile(file) {
    ...
    if (!ok) {
      toast("Only CSV and Excel files are accepted", false);
      return;
    }
  ...
  async function addContactsImportDo() {
    ...
    if (!file || !S.flow.importPreview?.ready) {
      toast("Choose a CSV or Excel file to import", false);
      return;
    }
    ...
    try {
      if (api.useMocks()) {
        ...
        S.flow.importResult = {
          validCount: added,
          errorCount: Math.max(0, lines.length - 1 - added),
          errors: [],
        };
        S.flow.importBusy = false;
        S.flow.importStage = null;
  ...
  async function contactSave(id) {
    ...
    const payload = {
      name: [
        document.getElementById("contact-first").value.trim(),
        document.getElementById("contact-last").value.trim(),
      ]
        .filter(Boolean)
        .join(" "),
    ...
    try {
      const updated = api.useMocks()
        ? {
            ...
            loc: [address.city, address.state, address.country].filter(Boolean).join(", "),
            city: address.city,
            state: address.state,
      ...
      closeLayer();
      toast("Recipient details saved");
      render();
    } catch (err) {
      toast(err.message || "Failed to save recipient details");
    }
  }
  ...
  function ViewIntegrations() {
    const apps = [
      ["Darwinbox", "HRIS", "Sync employees, birthdays & start dates", true],
      ["Keka", "HRIS", "People data & org chart", false],
      ["Razorpay", "Payments", "Collect funds via UPI, cards, netbanking", true],
      ["Shiprocket", "Logistics", "Domestic & global fulfilment tracking", true],
      ["Slack", "Comms", "Celebrate milestones in channels", false],
      ["Google Workspace", "Directory", "Provision recipients from Google", false],
    ];
  ...
  <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(280px,1fr))">${apps
    .map(
      ([n, t, d, on]) => `<div class="card" style="padding:18px">
    ...
    <button class="btn ${on ? "btn-ghost" : "btn-soft"} btn-sm btn-block" style="margin-top:14px" data-act="toast" data-arg="${on ? n + " settings" : "Connecting " + n + "…"}">${on ? "Manage" : "Connect"}</button></div>`,
    )
    .join("")}</div>`;
  }
...
function ViewCatalog(){
  ...
  function orderOpen(id) {
    ...
    const items = o.items
      .map(
        ([n, q]) =>
          `<div class="row" style="justify-content:space-between;border:1px solid var(--line);border-radius:var(--r-sm);padding:11px 14px;margin-bottom:8px"><span style="font-weight:600">${esc(n)}</span><span class="muted">Qty ${q}</span></div>`,
      )
      .join("");
  ...
  function orgNextColor() {
    const o = S.org;
    o._c = o._c || 0;
    const c = ORG_FALLBACK[o._c % ORG_FALLBACK.length];
    o._c++;
    return c;
  }
  ...
  function ViewWallets() {
    ...
    const n = S.org.step;
    const stepper = `<div class="org-stepper">${ORG_STEPS.map((lab, i) => {
      const s = i + 1;
      const cls = s < n ? "done" : s === n ? "active" : "";
      return `<div class="org-step ${cls}"><button class="sbtn" data-act="orgGo" data-arg="${s}"><div class="snum">${s < n ? "✓" : s}</div><div class="smeta"><span class="seye">Step ${s}</span><span class="slabel">${lab}</span></div></button>${s < 5 ? '<div class="sline"></div>' : ""}</div>`;
    }).join("")}</div>`;
  ...
  function entityManagerDashboard() {
    ...
    const alloc = dept.allocated || 0,
      spent = dept.spent || 0,
      rem = alloc - spent;
    const pendingNote = !alloc
      ? `<div class="banner" style="margin-bottom:18px;background:var(--warn-tint,#fff8e6);color:var(--ink-2);border:none"><b>Budget not allocated yet.</b> Your admin assigned you as manager, but the department budget has not been saved. Ask them to open <b>Wallets → Re-allocate budget</b> and finish setup.</div>`
      : "";
  ...
  function orgDashboard() {
    ...
    const statusTag = walletLive
      ? '<span class="tag tag-live"><span class="dot"></span>Active</span>'
      : '<span class="tag tag-draft">Setup in progress</span>';
    const setupBanner = walletLive
      ? ""
      : `<div class="banner" style="margin-bottom:18px">${I.wallet.replace('width="24" height="24"', 'width="16" height="16"')}<div><b>Wallet setup is not finished.</b> Complete allocation and manager invites, then activate from the review step. <span class="lnk" data-act="orgStart">Continue setup</span></div></div>`;
    const depts = S.org.departments,
      invited = depts.filter((d) => d.mgr.invite && d.mgr.email).length;
    const allocTot = alloc || 1;
    let acc = 0;
    const stops =
      depts
        .map((d) => {
  ...
  function orgWLive(e) {
    const el = e.target || e,
      id = el.id,
      o = S.org.wallet;
    if (id === "org-wamt" || id === "org-payamt") {
      const v = parseAmt(el.value);
      el.value = v ? v.toLocaleString("en-IN") : "";
      o.amount = v;
    ...
    const s1 = document.getElementById("org-sname");
    if (s1) s1.textContent = o.name || "Untitled wallet";
    const s2 = document.getElementById("org-sbudget");
    if (s2) s2.textContent = inr(o.amount);
    const s3 = document.getElementById("org-svalidity");
    if (s3) s3.textContent = fmtDate(o.start) + " → " + fmtDate(o.end);
  }
  function orgWChange(el) {
    if (el.id === "org-doctype") S.org.wallet.docType = el.value;
  }
  function orgFunding(m) {
    S.org.wallet.funding = m;
    render();
  }
  function orgPay(el, a) {
    S.org.wallet.pay = a;
    render();
  }
  function orgUpload() {
    S.org.wallet.uploaded = true;
    toast("Document uploaded · pending review");
    render();
  }
  function orgUploadClear() {
    S.org.wallet.uploaded = false;
    render();
  }
  ...
  function orgStep2() {
    ...
    const sugg = ORG_SUGG.map(
      (s) =>
        `<button class="chip ${used.includes(s.toLowerCase()) ? "used" : ""}" data-act="orgQuickAdd" data-arg="${esc(s)}"><span class="plus">+</span> ${s}</button>`,
    ).join("");
    const cards = S.org.departments
      .map(
        (d) => `<div class="dept-card">
  ...
  function orgQuickAdd(name) {
    if (S.org.departments.some((d) => d.name.toLowerCase() === name.toLowerCase())) return;
    const descs = {
    ...
    S.org.departments.push({
      id: S.org.seq++,
      name,
      desc: descs[name] || "Department merchandise and campaigns.",
      users: 10,
      allocated: 0,
      color: orgNextColor(),
      mgr: {
        ...
        email: "",
        mobile: "",
        role: ORG_ROLES.find((r) => r.startsWith(name)) || "Operations Manager",
        invite: true,
      },
    });
    toast(name + " added");
  ...
  function orgDeptDel(id) {
    if (S.org.departments.length <= 1) {
      toast("Keep at least one department", false);
      return;
    }
    S.org.departments = S.org.departments.filter((d) => String(d.id) !== String(id));
    render();
  }
  function orgDeptModal(arg) {
    const edit = arg !== "new";
    const d = edit ? orgDeptById(arg) : null;
    S.org.editId = edit ? String(arg) : null;
  ...
  function orgDeptSave() {
    const name = (document.getElementById("org-mname").value || "").trim();
    if (!name) {
      toast("Enter a department name", false);
      return;
    }
    const desc =
      (document.getElementById("org-mdesc").value || "").trim() ||
      "Department merchandise and campaigns.";
    const users = parseInt(document.getElementById("org-musers").value) || 1;
    if (S.org.editId) {
      const d = orgDeptById(S.org.editId);
      if (d) {
        d.name = name;
        d.desc = desc;
        d.users = users;
  ...
  /* ---- Step 3: allocate ---- */
  function orgStep3() {
    const total = S.org.wallet.amount,
      alloc = orgTotalAlloc(),
      rem = total - alloc;
    const segs =
      S.org.departments
        ...
        .map(
          (d) =>
            `<div class="alloc-seg" style="width:${Math.min((d.allocated / total) * 100, 100)}%;background:${d.color}">${(d.allocated / total) * 100 >= 8 ? Math.round((d.allocated / total) * 100) + "%" : ""}</div>`,
        )
        .join("") +
      (rem > 0
        ? `<div class="alloc-seg gap" style="width:${(rem / total) * 100}%">${(rem / total) * 100 >= 8 ? "Unallocated" : ""}</div>`
        : "");
    const legend = S.org.departments
      .map(
        (d) =>
          `<div class="leg"><span class="lc" style="background:${d.color}"></span>${esc(d.name)} · ${inr(d.allocated)}</div>`,
      )
      .join("");
    const rows = S.org.departments
      .map(
        (
          d,
        ) => `<tr><td><div class="row" style="gap:8px;align-items:center"><span class="lc" style="width:10px;height:10px;border-radius:3px;background:${d.color}"></span>${esc(d.name)}</div></td>
    ...
    <td class="r"><span class="pct-pill" id="org-pct-${d.id}">${total ? ((d.allocated / total) * 100).toFixed(((d.allocated / total) * 100) % 1 ? 1 : 0) : 0}%</span></td></tr>`,
      )
      .join("");
  ...
  function orgAllocLive(e) {
    const el = e.target;
    const d = orgDeptById(el.dataset.id);
    if (!d) return;
    let n = parseAmt(el.value);
    el.value = n ? n.toLocaleString("en-IN") : "";
    d.allocated = n;
    orgAllocRecalc();
  }
  function orgAllocRecalc() {
    const total = S.org.wallet.amount,
      alloc = orgTotalAlloc(),
      rem = total - alloc,
      over = alloc > total;
    ...
    set("org-stAlloc", inr(alloc));
    set("org-footAmt", inr(alloc));
    set("org-footPct", (total ? Math.round((alloc / total) * 100) : 0) + "%");
    const r = document.getElementById("org-stRem");
    if (r) {
      r.textContent = inr(rem);
      r.style.color = rem < 0 ? "var(--danger)" : "var(--brand-d)";
    }
    S.org.departments.forEach((d) => {
      const p = document.getElementById("org-pct-" + d.id);
      if (p)
        p.textContent =
          (total
            ? ((d.allocated / total) * 100).toFixed(((d.allocated / total) * 100) % 1 ? 1 : 0)
            : 0) + "%";
    });
    const bar = document.getElementById("org-bar");
    if (bar) {
      let segs = S.org.departments
        .filter((d) => d.allocated > 0)
        .map(
          (d) =>
            `<div class="alloc-seg" style="width:${Math.min((d.allocated / total) * 100, 100)}%;background:${d.color}">${(d.allocated / total) * 100 >= 8 ? Math.round((d.allocated / total) * 100) + "%" : ""}</div>`,
        ...
        .join("");
      if (rem > 0)
        segs += `<div class="alloc-seg gap" style="width:${(rem / total) * 100}%">${(rem / total) * 100 >= 8 ? "Unallocated" : ""}</div>`;
      bar.innerHTML = segs || '<div class="alloc-seg gap" style="width:100%">Unallocated</div>';
    }
    const lg = document.getElementById("org-legend");
    if (lg)
      lg.innerHTML = S.org.departments
        .map(
          (d) =>
            `<div class="leg"><span class="lc" style="background:${d.color}"></span>${esc(d.name)} · ${inr(d.allocated)}</div>`,
        )
        .join("");
    const al = document.getElementById("org-allocAlert");
    if (al) al.style.display = over ? "flex" : "none";
    document.querySelectorAll(".alloc-input").forEach((i) => i.classList.toggle("over", over));
    const nb = document.getElementById("org-next");
    if (nb) nb.disabled = over;
  }
  function orgSplitEven() {
    const total = S.org.wallet.amount,
      n = S.org.departments.length,
      base = Math.floor(total / n / 1000) * 1000;
    S.org.departments.forEach(
      (d, i) => (d.allocated = i === n - 1 ? total - base * (n - 1) : base),
    );
    toast("Budget split evenly");
    render();
  }
  ...
  function orgStep4() {
    return (
      `<div style="margin-bottom:14px"><h3 style="font-size:18px">Assign department managers</h3><p class="muted" style="font-size:13px;margin-top:2px">Managers control campaigns, recipients and spending for their department. Invitations are sent the moment setup completes.</p></div>` +
      S.org.departments
        .map(
          (d) => `<div class="mgr-card">
  ...
  function orgDeptById(id) {
    return S.org.departments.find((d) => String(d.id) === String(id));
  }
  function orgMgrLive(e) {
    const el = e.target;
    const d = orgDeptById(el.dataset.id);
    if (d) d.mgr[el.dataset.f] = el.value;
  }
  function orgMgrRole(el) {
    const d = orgDeptById(el.dataset.id);
    if (d) d.mgr.role = el.value;
  }
  function orgInvite(id) {
    const d = orgDeptById(id);
    if (!d) return;
    d.mgr.invite = !d.mgr.invite;
    render();
  }
  ...
  function orgStep5() {
    const o = S.org.wallet,
      total = o.amount,
      allocTot = orgTotalAlloc() || 1;
    let acc = 0;
    const stops = S.org.departments
      .map((d) => {
        const pct = (d.allocated / allocTot) * 100;
        const seg = `${d.color} ${acc}% ${acc + pct}%`;
        acc += pct;
  ...
  function orgNext() {
    const st = S.org.step;
    if (st === 3 && orgTotalAlloc() > S.org.wallet.amount) {
      toast("Reduce allocations to continue", false);
      return;
    }
    if (st === 5 && orgTotalAlloc() <= 0) {
      toast("Allocate budget to at least one department before finishing", false);
      return;
    }
    if (st === 5) {
      orgFinishConfirm();
  ...
  function orgFinishConfirm() {
    openModal(`<div class="modal-pad"><div class="modal-h"><div><h2 style="font-size:22px;font-family:var(--disp)">Finish setup?</h2></div><button class="xbtn" data-act="closeLayer">✕</button></div>
...
async function orgFinish(){
  ...
  function orgDone() {
    const o = S.org.wallet;
    const invited = S.org.departments.filter((d) => d.mgr.invite && d.mgr.email).length;
    const inviteCards = (S.org.sentInvites || [])
  ...
  function schedSet(el) {
    S.flow.sched = S.flow.sched || {};
    S.flow.sched[el.dataset.k] = el.value;
  }
  function prevTab(a) {
    S.flow.prevView = a;
    render();
  ...
  function artPrevThumb(i, u, sel) {
    const ph = u.preview
      ? `<img src="${u.preview}" alt="" style="width:100%;height:100%;object-fit:contain;display:block">`
      : LOGO_DECO;
  ...
  function artPick(el) {
    const i = +el.dataset.arg;
    const u = (S.artUploads || [])[i];
    if (!u) return;
    S.flow.artwork = true;
    S.flow.artSel = i;
    S.flow.artFile = { name: u.name, size: u.size, ext: u.ext, preview: u.preview, file: u.file };
    render();
  }
  ...
  function prevThumb(i, u, sel, act) {
    return `<div class="thumb ${sel ? "on" : ""}" data-act="${act}" data-arg="${i}"><div class="ph">${u.deco === "crest" ? I.spark.replace("currentColor", "#d33b30").replace('width="24" height="24"', 'width="30" height="30"') : LOGO_DECO}</div><div class="nm">${u.nm}</div></div>`;
  }
...
async function auth(){
  ...
  function shopOpen(id) {
    go("shopDetail", { flow: { shopId: id, shopTab: "Branded Swag" }, nav: "shops" });
  }
  function shopTab(t) {
    S.flow.shopTab = t;
    render();
  ...
  function swagView(mode) {
    S.flow.swagView = mode;
    render();
  }
  ...
  function userMenu() {
    ...
    <div style="border-top:1px solid var(--line);padding-top:8px">
      ${[
        ...
        .map(
          ([l, v]) =>
            `<div class="nav-item" style="border-radius:var(--r-sm)" data-act="navClose" data-arg="${v}">${I.gear}<span>${l}</span></div>`,
        )
        .join("")}
...
const CHANGED = { schedSet:1, singleLocLive:1, orgWChange:1, orgMgrRole:1 };              // change-driven
  ...
  function onShelfMerchClick(e) {
    ...
    if (e.target.hasAttribute && e.target.hasAttribute("data-scrim")) {
      closeLayer();
      return;
    }
    ...
    if (
      ...
      act === "swagCardMenu" ||
      act === "collectionMenu" ||
      t.classList?.contains("swag-card-menu")
    )
      e.stopPropagation();
    fn(t, t.dataset.arg);
  }
  ...
  document.addEventListener("dragover", function (e) {
    if (
      e.target.closest("#sh-logo-drop") ||
      e.target.closest("#kt-logo-drop") ||
      e.target.closest("#sw-art-drop") ||
      e.target.closest("#shop-edit-logo-drop") ||
      e.target.closest("#ac-import-drop")
    )
      e.preventDefault();
  });
